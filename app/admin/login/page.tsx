...
export default function AdminLoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true)
    try {
      const { token } = await adminApi.login(email, senha)
      login(token)
      router.push('/admin')
    } catch (err: unknown) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-7">
          ...
          <p className="text-muted text-sm mt-1">Back-office (admin)</p>
        </div>

        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-2xl shadow-card p-6 space-y-4">
          <Field icon={Mail} type="email" value={email} onChange={setEmail} placeholder="admin@email.com" label="E-mail" />
